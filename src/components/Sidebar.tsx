...
export const Sidebar = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const cngStation = findBy((a) => (a.name || "").toLowerCase() === "cng station" || (a.menuName || "").toLowerCase() === "cng station");
        ...
        if (cngStation) dyn.push({ id: "assets:cng", label: normalizeHeading(cngStation.menuName || cngStation.name), icon: Gauge, order: cngStation.menuOrder ?? 4, path: "/assets/cng" });
        if (catastrophe) dyn.push({ id: "assets:catastrophe", label: normalizeHeading(catastrophe.menuName || catastrophe.name), icon: AlertTriangle, order: catastrophe.menuOrder ?? 5, path: "/assets/catastrophe" });
  ...
  const filteredAssetMenus = assetMenus.filter((m) => {
    if (userRole === "survey") {
      return m.id !== "assets:pipeline" && m.id !== "assets:valve" && m.id !== "assets:consumer" && m.id !== "assets:cng" && m.id !== "assets:catastrophe";
    }
    return true;
  });
